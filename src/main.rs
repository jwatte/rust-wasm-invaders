...
async fn main() {
    ...
    loop {
        state::update_state(&mut state, &assets);
...
fn render_scene(state: &state::State, assets: &assets::Assets) {
...

    {
        ...
        for n in 0..state.lives {
            sprite::draw_sprite(left, top, width, pspr, (n as f32 + 0.6) * pspr.f_w * 0.5, 1.333-pspr.f_h*0.6*0.45, 0.0, 0.45);
        }

        //  draw player
        sprite::draw_sprite(left, top, width, pspr, state.player_pos_fr, 1.333-0.06, 0.0, 1.0);
    }

    //  draw aliens
    for alien in state.aliens.iter() {
        let asp = &assets.sprites[alien.sprite.index];
        sprite::draw_sprite(left, top, width, asp, alien.xpos, alien.ypos, alien.phase.sin()*0.1, 1.0);
    }

    {
        ...
        for bullet in state.bullets.iter() {
            sprite::draw_sprite(left, top, width, bspr, bullet.xpos, bullet.ypos+bspr.f_h*0.5, 0.0, 1.0);
        }
    }
    ...
    if state.paused {
        ...
        mq::draw_text(&pdisp, left + width * 0.25, top + height * 0.2, height * 0.04, mq::WHITE);
    } else if state.reset_countdown > 0.0 {
        let ctdisp = format!("Countdown: {:.1}", state.reset_countdown);
        mq::draw_text(&ctdisp, left + width * 0.3, top + height * 0.2, height * 0.04, mq::WHITE);
    }
}
